...
🔺**静态数据成员**：属于类本身，所有对象共享同一份类中的静态变量，不属于任何单个对象
...
class Singleton {
private:
    static Singleton instance;   // 类内声明静态成员对象
...
Singleton Singleton::instance;   // 类外定义静态成员对象
```

🔺**步骤**：

1. 在类内声明一个静态成员对象，表示这份对象属于类本身，所有访问都共享它
2. 将构造函数设为 `private`，禁止类外直接创建对象
3. 删除拷贝构造和赋值运算符，防止复制出第二份对象
4. 提供公共静态成员函数，专门返回这唯一对象
5. 在类外完成静态成员对象的定义

🔺**优点**：

- **唯一性明确**：整个程序只有一个实例
- **共享方便**：所有使用者访问的都是同一份对象
- **入口统一**：通过 `getInstance()` 获取，调用方式固定
- **节省资源**：避免重复创建多个功能相同的对象
- **适合全局管理类**：如日志器、配置管理器、设备管理器

🔺**本质**：静态成员保证“类里只有这一份”，私有构造保证“外面不能再 new 出第二份”
